...
####################################################### cleaned data set
raw_data <- read.csv("/Users/jonathanzeh/Library/CloudStorage/OneDrive-Personal/RStudio/BA_Thesis/merged_data_sets/stoxx_europe_50_merge_raw.csv")
...
for (i in 1:max(df_subsets$index_vector, na.rm = TRUE)) {
  ...
  # Generate and save the plot
    ...
    labs(title = 
           paste("Stock Weights Over Time - iShares euro STOXX 50 ETF",
...
# check weights
```{r, eval = F, include = F}
weight_df <- cleaned_data %>%
  group_by(date) %>%
  summarise(sum(weight))
colnames(weight_df) = c("date", "sum_weight")

weight_df <- weight_df[order(weight_df$sum_weight), ]
head(weight_df)

ggplot(weight_df) +
  geom_line(aes(x = date, y = sum_weight)) +
  geom_hline(yintercept = 100, linetype = "dashed", color = "red") +
  ggtitle("sum of weights for each date")
