...
## correlation matrix
```{r}
cor_df <- as.data.frame(cor(df_summary))
...
data_on_fe_function <- function(df, indep_vars, fixed_vars, 
  ...
  generate_models <- function(period, df, indep_vars, fixed_vars, setup) {
    ...
    if(setup == "single"){
      ...
      models_list[['fe_model']] <- fe_function(
      df, indep_vars, fixed_vars, period, volatility_lag = FALSE)
    } else {
      models_list[['fe_model']] <- fe_function(
      df, indep_vars, fixed_vars, period, volatility_lag = FALSE,
    ...
    if(setup == "lag" || setup == "lag_fund") {
      ...
      models_list[['fe_model_lag']] <- fe_function(
        df, indep_vars, fixed_vars, period, volatility_lag = TRUE,
        fund_consolidation = FALSE)
    }
    ...
    if(setup == "fund" || setup == "lag_fund") {
      ...
      models_list[['fe_model_fund']] <- fe_function(
        df, indep_vars, fixed_vars, period, 
...
style_df <- style.df(depvar.title = "", fixef.title = "", #fixef.line = "-",
                     stats.title = "", #stats.line = "-",
                     yesNo = c("Yes", "No"))

etable_function <- function(fe_models_list, title, 
                               subfolder, file_name) {
...
fe_models_list_1 <- data_on_fe_function(
  df_outstanding, indep_vars, fixed_vars, periode_list, single_setup = TRUE)
...
etable_function(combined_list,
                 ...
                 Period: 2010 to 2024",
                   ...
                   "full_sample_df_outstanding.tex")
```
...
fe_models_list_5 <- data_on_fe_function(
  df,indep_vars, fixed_vars, periode_list, setup = "lag_fund")

etable_function(fe_models_list_5,
                   "Data: df,
                 Periods: 2010 to 2019, 2020, 2021 to 2023",
                   "sample_split_regressions",
                   "sample_split_df.tex")

#################################################################
fe_models_list_6 <- data_on_fe_function(
  df_outstanding, indep_vars, fixed_vars, periode_list, setup = "lag_fund")

etable_function(fe_models_list_6,
                   "Data: df_outstanding,
                 Periods: 2010 to 2019, 2020, 2021 to 2023",
                   "sample_split_regressions",
                   "sample_split_df_outstanding.tex")
...
periode_list <- list(c(2010, 2015), c(2015, 2020), c(2020, 2024))

fe_models_list_6 <- data_on_fe_function(df_outstanding,
                                        indep_vars, fixed_vars,
                                        periode_list, setup = "lag_fund")
etable_function(fe_models_list_6,
                   "Data: df_index,
                 Periods: 2010 to 2014, 2015 to 2019, 2020 to 2023",
                   "sample_split_regressions",
                   "sample_split_2_df_outstanding.tex")
```
...
fe_models_list_7 <- data_on_fe_function(df,
                                        indep_vars, fixed_vars,
                                        periode_list, setup = "lag")
