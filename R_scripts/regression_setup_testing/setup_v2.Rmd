...
```{r}
########### v1
fe_models_list <- data_on_fe_function(df) 
summary(fe_models_list[1])
fe_models_list[[1]]
```
...
latex_code <- stargazer(fe_models_list[[4]],
          ...
          type = "latex",
          title = "First Results Jonathan",
          dep.var.labels = c("Daily Volatility (t)"),
          column.labels = c("2010 to 2019", "2020", "2021 to 2023"),
          covariate.labels = 
            c("ETF ownership", "log(Mktcap t-1)", "1/Price t-1", "Amihud t-1",
              "Bid-ask spread t-1", "Past 12-month returns t-1", 
              "Fund ownership t-1", "Volatility t-1", 
              "Volatility t-2", "Volatility t-3"),
          omit.stat = c("f", "ser"),
          add.lines = list(c("Month fixed effects", 
                             "Yes", "Yes", "Yes"),
                           c("Stock fixed effects", 
                             "Yes", "Yes", "Yes")),
          out = "/Users/jonathanzeh/Library/CloudStorage/OneDrive-Personal/BA_Thesis/BA_coding/R_exports/regression_outputs/regression_table_sample_split_1_laggs.tex")
...
```{=latex, include = F, eval = F}
\begin{landscape}
`r cat(latex_code, sep = "\n")`
\end{landscape}
```
...
stargazer(fe_model_2009_2019, fe_model_2009_2019_lag,
          fe_model_2020, fe_model_2020_lag,
          fe_model_2021_2023, fe_model_2021_2023_lag,
          ...
          covariate.labels = 
            c("ETF ownership", "log(Mktcap t-1)", "1/Price t-1", "Amihud t-1",
              "Bid-ask spread t-1", "Book-to-market t-1", 
              "Past 12-month returns t-1", 
              "Fund ownership t-1", "Volatility t-1", 
              "Volatility t-2", "Volatility t-3"),
          omit.stat = c("f", "ser"),
          add.lines = list(c("Month fixed effects", 
                           ...
                           c("Stock fixed effects", 
                             "Yes", "Yes", "Yes", "Yes", "Yes", "Yes")),
          out = "/Users/jonathanzeh/Library/CloudStorage/OneDrive-Personal/BA_Thesis/BA_coding/R_exports/regression_outputs/regression_table.tex")
...
# Define separate covariate labels for models with and without lagged variables

covariate_labels <- c(
  "ETF ownership", "log(Mktcap t-1)", "1/Price t-1", "Amihud t-1",
  "Bid-ask spread t-1", "Past 12-month returns t-1",
  "Fund ownership t-1", "Volatility t-1", "Volatility t-2", "Volatility t-3"
)

# Run stargazer with appropriate covariate labels
latex_code <- stargazer(
  fe_model_2009_2019, fe_model_2009_2019_lag,
  fe_model_2020, fe_model_2020_lag,
  fe_model_2021_2023, fe_model_2021_2023_lag,
  
  title = "OLS Regressions, Sample Split by Period",
  type = "latex",
  style = "AER",
  column.labels = c("2009 to 2019", "2020", "2021 to 2023"),
  dep.var.labels.include = FALSE,
  dep.var.caption = "Daily Volatility (t)",
  
  covariate.labels = covariate_labels,
  
  omit = c("Volatility t-1", "Volatility t-2", "Volatility t-3"),
  omit.labels = c("1", "3", "5"),
  intercept.bottom = FALSE,
  omit.stat = c("f", "ser"),
  add.lines = list(
    c("Month fixed effects", "Yes", "Yes", "Yes"),
    c("Stock fixed effects", "Yes", "Yes", "Yes")),
  
  out = "/Users/jonathanzeh/Library/CloudStorage/OneDrive-Personal/BA_Thesis/BA_coding/R_exports/regression_outputs/regression_table_test.tex"
)
